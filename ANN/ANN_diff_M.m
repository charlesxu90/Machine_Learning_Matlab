...
max_iter = 50000; % Maximum iteration for training
eta = 0.2; % Learning rate
no_ini = 1; % No. of different initialization
% M is No. of hidden layer nodes
for M= 3:12
    W1=ones(D+1,M);
    W2=ones(M+1,K);
...
figure(1)
errorbar(3:12,train_err_mean,train_err_std,'rp-')
hold on
errorbar(3:12,test_err_mean,test_err_std,'bs-')

legend('Train error', 'Test error');
grid on;
xlabel('Different number of hidden nodes');
ylabel('Error');
title('Error on test and train data with number of hidden nodes');
